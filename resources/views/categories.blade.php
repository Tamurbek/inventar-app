            ...
            @forelse ($categories as $category)
                <tr>
                    ...
                    <td class="border border-gray-300 px-4 py-2">
                        @if($category->image)
                            <img src="{{ asset($category->image) }}" alt="Image" style="width: 100px; height: auto;">
                        @endif
                    </td>
                    ...
                    <td class="border border-gray-300 px-4 py-2 text-center">
                        <button onclick="openModal(
                                                     {{ $category->id }}, 
                                                    '{{ $category->name }}',
                                                    '{{ $category->image }}')" class="text-blue-500 hover:underline">Tahrir</button> |
                        <form action="{{ route('category.delete', $category->id) }}" method="POST" class="inline-block">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="text-red-500 hover:underline" onclick="return confirm('Are you sure you want to delete this category?')">O`chirish</button>
                        </form>
                    </td>
                </tr>
            @empty
                <tr>
                    <td colspan="" class="text-center text-gray-500 py-4">Ro`yxat bo`sh</td>
                </tr>
            @endforelse
        </tbody>
    </table>
